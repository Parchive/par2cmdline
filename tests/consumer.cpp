...
public:
  ...
  void OnSetInfo(const par2::Par2SetInfo &info) { ++setinfo; lastinfo = info; }
  void OnFile(const std::string &) { ++files; }
  void OnFileDone(const std::string &, par2::u32, par2::u32) { ++done; }
  void OnRepairStart(void) { ++repairs; }
  void OnError(const par2::Par2Error &error) { ++errors; lasterror = error; }
  void OnWarning(const par2::Par2Warning &warning) { ++warnings; lastwarning = warning; }
